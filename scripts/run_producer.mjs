...
// ── 3. 使用现有 User + Book（按 filePath 查找书籍）──
const {
  UserRepository,
  BookRepository,
  CharacterRepository,
...
if (!producerEmail) {
  throw new Error('未配置 PRODUCER_USER_EMAIL，制片人管道已停止');
}
const user = await UserRepository.findByEmail(producerEmail);
if (!user) {
  throw new Error('制片人账号不存在，请先注册后再运行管道');
}
