/**
 * Merge all 4 batches and regenerate full entity output
 */
...
function mergeCh(a: Ch, b: Ch): Ch {
  ...
  return { ...base, aliases: unique([...(base.aliases || []), ...(other.aliases || []), other.name]).filter(al => al !== base.name),
    description: base.description || other.description, confidence: Math.max(a.confidence ?? 0, b.confidence ?? 0),
  ...
  for (const item of arr) {
    ...
    if (!e) { map.set(key, { ...item }); } else {
      ...
      map.set(key, { ...e, aliases: unique([...(e.aliases || []), ...(item.aliases || [])]).filter(al => al !== e.name),
        description: e.description || item.description, confidence: Math.max(e.confidence ?? 0, item.confidence ?? 0),
...
async function main() {
  ...
  // Write output
  const ts = Date.now();
  const bookDir = resolve('output', `斗破苍穹-100章-${ts}`);
  ...
  for (const c of dCh) {
    ...
    const d = (c.description||'').replace(/\|/g,'\\|').slice(0,80);
  ...
  for (const i of dIt) {
    ...
    const d = (i.description||'').replace(/\|/g,'\\|').slice(0,80);
  ...
  for (const l of dLo) {
    ...
    const d = (l.description||'').replace(/\|/g,'\\|').slice(0,80);
