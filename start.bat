...
:: 静默模式（桌面快捷方式用）：无窗口后台运行，日志写入 logs\，失败时弹窗提示
set "SILENT="
...
if defined SILENT (
    ...
    if errorlevel 1 (
        echo [错误] API 服务后台启动失败。
        call :on_error
        exit /b 1
    )
    echo       API 服务已在后台启动：http://localhost:3001（日志：logs\api.log）
) else (
    cd /d "%~dp0api"
...
if defined SILENT (
    ...
    start "" http://localhost:5173
    echo.
    echo   启动完成，服务正在后台隐藏运行。
    echo   停止服务请运行 stop.bat，日志在 logs\ 目录。
    exit /b 0
)
...
:show_error_popup
powershell -NoProfile -WindowStyle Hidden -Command "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show('群像启动失败，已自动打开日志文件，请根据提示处理后重试。','群像','OK','Error')"
exit /b 0
