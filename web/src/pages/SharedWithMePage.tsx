...
export function SharedWithMePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <Card className="p-10 text-center text-sm text-muted-foreground">暂无分享</Card>
      ) : (
        <div className="grid gap-3">
          {items.map((s) => {
            const st = STATUS[s.status] ?? { label: s.status, variant: 'muted' as const };
            return (
              <Card key={s.shareId} className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {s.status === 'copying' && <span className="text-xs text-muted-foreground">复制中…</span>}
                  {s.status === 'copied' && (
