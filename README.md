...
群像覆盖从原文导入、结构识别、实体与世界观提取，到审核、视觉设定、故事资产和版本交付的完整流程。产品提供中文工作台、账号与书库、多人分享、多密钥并发、文本编码兼容和稳定的任务管线。

## 产品定位

- 把长篇文本转化为具有证据来源和版本记录的结构化数字资产。
- 让人物、地点、道具、技能、世界观和故事信息可以集中审核、修订与复用。
- 为视觉生成、故事制作和后续内容生产提供稳定的数据契约与导出能力。
- 通过本地优先的运行方式和可选对象存储，兼顾个人使用与服务化部署。

## 音频协作与产品边界

按[音频组定位（2026-09-08，83aca80）](https://github.com/Linzeyu912/YingHe-music/blob/83aca809a02612d3b7cd3a4a7c84705182a99637/README.md)，音频组在前期与实体组共同准备人物音色，交付音色描述、参考样本、角色绑定关系和版本，纳入群像的人物资产准备；在后期检查视频吐字清晰度与跨分镜配乐连续性，处理配乐衔接、淡入淡出和混音，由视频二组回填时间线并完成最终合成。

情绪对白制作继续保留：与视频一组协作，按需先制作对白、试听调整满意后再生成视频。独立环境音、动作/打斗音效制作，以及对已生成视频重新配音的适用范围，仍待与老师确认，不列为每段视频的必经步骤。

群像承接实体与故事组经验证的能力，并规划纳入上述人物音色资产；声影承接下游音视频制作与合成能力。音频研发与声音资产以 `YingHe-music` 为权威来源。这里描述的是协作安排，不能据此认定群像已完成音色绑定接口，或声影已经发布全部研发能力；交接必须记录来源仓库、路径、提交号和资产版本。
...
- 保留故事切分、边界审核、故事资产包和导演流程的工程入口；故事策划新主链仍以 `YingHe-story` 的实现与验收为准，不能据此认定已整体集成。
- 本地文件系统对象存储开箱即用；生产环境可切换到 MinIO、R2、AWS S3、OSS 等 S3 兼容服务。

## 已核对的进展

- 核对基线：`QunXiang` 的 `6f74090`（2026-09-04）；本次文档核对日期为 2026-09-08。
- 8 月底至 9 月初已加入实体别名清洗、补跑合并修正、重跑残留清理，以及管线终态与视觉设定生成修复。
- 审核页“全部状态”使用 `ALL` 审核集合，包含已拒绝实体；相关实现和回归用例已入库。
- 文档核对不等于本次重新运行完整模型流程；现有验证入口见下文。
