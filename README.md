...
开发过程中使用了若干公开网络小说（如《斗破苍穹》《凡人修仙传》等）作为测试语料，用来验证抽取流水线在长篇、多章节、人物关系复杂场景下的效果。

**仓库中不包含任何小说原文或运行产出的原文摘录**——原始文本受版权保护，抽取结果中也可能包含直接引用原文的片段，均不适合公开分发。仓库只保留代码本身；如需复现测试效果，请自行准备已获授权的文本作为输入。
