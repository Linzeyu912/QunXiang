...
function lastDelimiterIndex(value) {
    return Math.max(value.lastIndexOf('，'), value.lastIndexOf(','), value.lastIndexOf('；'), value.lastIndexOf(';'), value.lastIndexOf('。'), value.lastIndexOf('！'), value.lastIndexOf('？'), value.lastIndexOf('!'), value.lastIndexOf('?'));
}
function looksIncompleteTrailingFragment(fragment) {
    const text = fragment.trim().replace(TRAILING_DELIMITERS_RE, '');
    if (!text)
        return true;
    return DANGLING_END_RE.test(text) || BROKEN_NUMERIC_TAIL_RE.test(text);
}
function trimIncompleteTrailingFragment(value) {
